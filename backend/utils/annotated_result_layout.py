...
_LCA_MARK_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "report_assets", "lahore-css-academy-mark.png"
)


def _report_brand() -> str:
    """Current report brand ('lca' or 'rubric') for this worker thread."""
    try:
        from backend.utils.report_cover import current_report_brand
        return current_report_brand()
    except Exception:
        return "rubric"
...
def draw_masthead(canvas: np.ndarray, meta: str = "") -> np.ndarray:
    ...
    # Brand logo tile + wordmark (LCA logo for the Lahore CSS Academy app, else Rubric.ai)
    brand = _report_brand()
    logo = max(28, int(0.028 * w))
    lx, ly = pad_x, pad_y
    wordmark = "RUBRIC.AI"
    if brand == "lca":
        wordmark = "LAHORE CSS ACADEMY"
        pasted = False
        try:
            lca = Image.open(_LCA_MARK_PATH).convert("RGBA").resize((logo, logo), Image.LANCZOS)
            pil.paste(lca, (lx, ly), lca)
            pasted = True
        except Exception:
            pasted = False
        if not pasted:
    ...
    # Wordmark + subtitle
    word_size = max(12, int(0.012 * w)) if brand != "lca" else max(11, int(0.0104 * w))
    word_font = _load_font("syne", word_size)
    sub_font = _load_font("syne", max(8, int(0.0075 * w)))
    tx = lx + logo + max(10, int(0.01 * w))
    draw.text((tx, ly + 2), wordmark, font=word_font, fill=_bgr_to_rgb(INK_BGR))
    draw.text(
...
def draw_footer(canvas: np.ndarray) -> np.ndarray:
    ...
    left = "GENERATED BY LAHORE CSS ACADEMY" if _report_brand() == "lca" else "GENERATED BY RUBRIC.AI"
