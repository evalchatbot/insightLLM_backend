...
async def test_direct_rag_processing():
    ...
    print("🔍 Testing Direct RAG Processing...")
    ...
    for question in test_questions:
        try:
            ...
            print(f"  ✓ '{question[:30]}...' → {len(result.get('answer', ''))} chars")
            assert result.get("answer") is not None
            assert "tool_used" in result.get("metadata", {})
            assert result["metadata"]["tool_used"] == "rag_tool"
        except Exception as e:
            print(f"  ⚠️ '{question[:30]}...' failed: {e}")
    
    print("✅ Direct RAG Processing tests passed!\n")


async def test_agent_capabilities():
    """Test agent capabilities and initialization."""
    print("🛠️ Testing Agent Capabilities...")
    ...
    print(f"  ✓ Processing approach: {processing['approach']}")
    print(f"  ✓ Classification: {processing['classification']}")
...
async def main():
    """Run all tests."""
    print("🚀 Starting Simplified RAG Architecture Tests\n")
    ...
    try:
        ...
        print("🎉 All tests passed! Simplified RAG architecture is working correctly.")
