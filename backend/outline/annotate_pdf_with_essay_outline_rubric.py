...
def _draw_spelling_error_on_canvas(
    canvas: np.ndarray,
    error: Dict[str, Any],
    page_ocr: Dict[str, Any],
    ...
    """
    Draw a spelling error annotation on the canvas.
    Returns True if drawn successfully, False otherwise.
    
    Draws:
    - Red rectangular box around the error word
    - White label with correction text above the box
    """
    error_text = error.get("error_text", "").strip()
    ...
    # Find the error word in OCR words using normalized matching
    error_bbox = None
    for word in words:
        ...
        if _norm_token_for_spelling(word_text) == _norm_token_for_spelling(error_text):
            bbox = word.get("bbox", [])
            if bbox:
                error_bbox = bbox
                break
    ...
    if extent:
        ...
        pts = _poly_to_points_generic(error_bbox)
        r = _points_to_rect(pts)
        if not r:
            return False
        
        # Scale by extent and shift
        rr = _scale_rect_by_extent(r, extent, orig_w, orig_h)
        rr = _shift_rect(rr, left_width, y_offset)
    else:
        # Use bbox directly if no extent
        pts = _poly_to_points_generic(error_bbox)
        if not pts:
            return False
        xs = [p[0] for p in pts]
        ...
        x0, x1 = min(xs), max(xs)
        y0, y1 = min(ys), max(ys)
        rr = (int(x0 + left_width), int(y0 + y_offset), int(x1 + left_width), int(y1 + y_offset))
    
    # Draw red rectangle around error (BGR color: red = (0, 0, 255))
    cv2.rectangle(canvas, (rr[0], rr[1]), (rr[2], rr[3]), (0, 0, 255), 2)
    
    # Draw white background rectangle for correction text
    font = cv2.FONT_HERSHEY_SIMPLEX
    font_scale = 1.2  # Increased from 0.6 for better readability
    thickness = 2  # Increased thickness for bold text
    (text_width, text_height), baseline = cv2.getTextSize(correction, font, font_scale, thickness)
    
    # Position text above the error box
    # Use the full width of the error box for the label
    text_y = max(rr[1] - 8, text_height + 4)  # 8 pixels above the box, but ensure it's on canvas
    
    # Make label match the exact width of the error box
    bg_x1 = rr[0] - 2  # Start at left edge of error box (with small padding)
    bg_x2 = rr[2] + 2  # End at right edge of error box (with small padding)
    bg_y1 = text_y - text_height - 4
    bg_y2 = text_y + 2
    
    # Ensure background doesn't go off canvas
    bg_y1 = max(0, bg_y1)
    bg_x2 = min(canvas.shape[1] - 1, bg_x2)
    
    # Draw white background (full width of error box)
    cv2.rectangle(canvas, (bg_x1, bg_y1), (bg_x2, bg_y2), (255, 255, 255), -1)
    cv2.rectangle(canvas, (bg_x1, bg_y1), (bg_x2, bg_y2), (0, 0, 255), 1)  # Red border
    
    # Center the correction text horizontally within the label
    # Calculate text width to center it
    text_actual_width = text_width
    label_width = bg_x2 - bg_x1
    text_start_x = bg_x1 + (label_width - text_actual_width) // 2
    
    # Draw correction text in black (centered in the label)
    cv2.putText(
        canvas,
        correction,
        (text_start_x, text_y),
...
def annotate_pdf_essay_pages(
    ...
    *,
    ...
    # Map OCR pages by page number (1-based) - do this before rendering to identify pages with content
    ...
    # Pages with OCR data
    for pn in ocr_pages_by_num.keys():
        if isinstance(pn, int) and pn >= 1:
            pages_with_content.add(pn)
    ...
    for pno in suggestions_by_page.keys():
        if isinstance(pno, int) and pno >= 1:
            pages_with_content.add(pno)
    ...
    # Only render pages with content to save time
    pil_pages: List[Optional[Image.Image]] = []
    for page_idx, page in enumerate(doc):
        page_number = page_idx + 1
        if page_number in pages_with_content:
            pix = page.get_pixmap(dpi=dpi)
            pil_pages.append(Image.open(io.BytesIO(pix.tobytes("png"))))
        else:
            # Skip empty pages - don't render
            pil_pages.append(None)

    RED = (0, 0, 255)
    # Build a list that maintains page order - None for skipped pages
    annotated_pages: List[Optional[Image.Image]] = [None] * len(doc)

    for page_idx, pil_img in enumerate(pil_pages):
        # Skip empty pages that weren't rendered
        if pil_img is None:
            continue
        page_number = page_idx + 1
        ...
        # Draw spelling/grammar errors on canvas
        page_spelling_errors = [e for e in spelling_errors if e.get("page") == page_number]
        for error in page_spelling_errors:
            _draw_spelling_error_on_canvas(
                canvas=canvas,
                error=error,
                page_ocr=page_ocr,
                extent=extent,
    ...
    # Return list maintaining page order (index 0 = page 1, index 1 = page 2, etc.)
    # None values represent skipped empty pages - callers should filter by page number
    return annotated_pages


# ============================================================
# OUTLINE ENTRY POINT (alias for outline pipeline)
# ============================================================
def annotate_pdf_outline_pages(
    pdf_path: str,
    ocr_data: Dict[str, Any],
    structure: Dict[str, Any],
    grading: Dict[str, Any],
    ...
    *,
    ...
    """
    Outline pipeline entry point. Forwards to annotate_pdf_essay_pages.
    """
    return annotate_pdf_essay_pages(
        pdf_path=pdf_path,
        ocr_data=ocr_data,
        structure=structure,
        grading=grading,
