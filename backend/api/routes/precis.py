...
from backend.precis.grade_pdf_precis import run_precis_grading
from backend.ocr.job_manager import OCRJobManager, JobStatus
from backend.ocr.progress_tracker import OCRProgressTracker

router = APIRouter(prefix="/api/precis", tags=["precis"])
logger = logging.getLogger(__name__)

# Use a separate job directory for precis jobs
...
def _get_precis_dir() -> str:
    """Get the precis directory where rubric files and assets live."""
    current_file_dir = os.path.dirname(os.path.abspath(__file__))
    return os.path.abspath(os.path.join(current_file_dir, "..", "..", "precis"))


def _cleanup_temp_folders():
    """Clean up old timestamped temp folders from precis processing."""
    try:
        precis_dir = _get_precis_dir()
        patterns_to_clean = [
            os.path.join(precis_dir, "debug_llm*"),
            os.path.join(precis_dir, "grok_images_precis*"),
            os.path.join(precis_dir, "__pycache__"),
        ]
        folders_to_clean = []
        ...
        logger.warning(f"Failed to clean up precis temp folders: {e}")


def _cleanup_old_results(max_age_hours: int = 24):
    """Delete result files older than max_age_hours."""
    try:
        results_dir = os.path.join(_get_logs_dir(), "precis_results")
        ...
        for filename in os.listdir(results_dir):
            ...
            if os.path.isfile(filepath) and os.path.getmtime(filepath) < cutoff_time:
                ...
                deleted_count += 1
        if deleted_count > 0:
            logger.info(f"Cleaned up {deleted_count} old precis result file(s)")
    except Exception as e:
        logger.warning(f"Failed to clean up old precis results: {e}")


def _process_precis_job(
    job_id: str,
    request_id: str,
    temp_dir: str,
    file_path: str,
    ...
    try:
        ...
        precis_dir = _get_precis_dir()
        rubric_docx = os.path.join(precis_dir, "Precis Rubric.docx")
        annotations_rubric_docx = os.path.join(precis_dir, "ANNOTATIONS RUBRIC FOR PRECIS.docx")
        colouring_scheme_image = os.path.join(precis_dir, "colouring_scheme.jpeg")
        extra_json_path = os.path.join(temp_dir, "extra_text.json")

        # The precis pipeline uses load_dotenv(env_file) — we point it to the
        # main project .env so all keys are picked up from one place.
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))
        env_file = os.path.join(project_root, ".env")

        result_data = run_precis_grading(
            pdf_path=file_path,
            output_json_path=str(output_json_path),
            output_pdf_path=str(output_pdf_path),
            rubric_docx=rubric_docx,
            annotations_rubric_docx=annotations_rubric_docx,
            env_file=env_file,
        ...
        # Re-save result if it's a dict
        if isinstance(result_data, dict):
            with open(str(output_json_path), "w", encoding="utf-8") as f:
                json.dump(result_data, f, ensure_ascii=False, indent=2)

        progress_callback(100, "Precis Evaluation Complete!")
        _job_manager.complete_job(job_id)
        logger.info(f"Precis job {job_id} completed successfully")

    except Exception as e:
        logger.error(f"Precis job {job_id} failed: {e}", exc_info=True)
        ...
        if os.path.exists(temp_dir):
            ...
            logger.info(f"Precis job {job_id} - Cleaned up temp dir: {temp_dir}")
...
async def submit_precis(
    ...
    if (pipeline or "").strip().lower() != "precis":
        raise HTTPException(
            status_code=400,
            detail="Invalid pipeline token for precis endpoint. Expected pipeline='precis'.",
        )
    ...
    temp_dir = f"temp_precis_job_{job_id}"
...
async def get_precis_status(job_id: str):
...
async def download_precis_pdf(job_id: str):
    """Download the annotated PDF for a precis job."""
    job = _job_manager.get_job(job_id)
    ...
    return FileResponse(
        path=pdf_path,
        filename=f"annotated_precis_{job_id}.pdf",
        media_type="application/pdf",
    )


@router.get("/result/{job_id}")
async def get_precis_result(job_id: str):
    job = _job_manager.get_job(job_id)
    if not job or job.status != JobStatus.COMPLETED:
    ...
    download_url = f"/api/precis/download/{job_id}"
